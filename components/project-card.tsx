...
import { CalendarIcon, Users, Hammer, ChevronRight, MapPin } from "lucide-react";
...
export function ProjectCard({ project, onRefresh }: ProjectCardProps) {
  ...
  return (
    <Card className="h-full flex flex-col overflow-hidden border-border/50 bg-card/50 backdrop-blur-sm transition-all duration-300 hover:border-primary/50 hover:shadow-[0_0_30px_-10px_rgba(124,58,237,0.2)] group cursor-pointer" onClick={() => router.push(`/project/${project.id}`)} data-testid={`card-project-${project.id}`}>
