...
export default function Home() {
  ...
  const filteredProjects = selectedDate
    ? projectsWithDates.filter(p =>
        isWithinInterval(selectedDate, { start: p.startDate, end: p.endDate }) ||
  ...
  return (
    <div className="min-h-screen w-full bg-background text-foreground relative overflow-x-hidden">
      <div className="fixed inset-0 z-0">
        <img
          src="/generated_images/dark_abstract_tech_background_with_gradients.png"
          alt="Background"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {filteredProjects.map(project => (
                  <div key={project.id} className="h-full">
                    <ProjectCard project={project} onRefresh={() => refetch()} />
                  </div>
                ))}
              </div>
            ) : projects.length === 0 ? (
